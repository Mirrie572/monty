...
void p_char(stack_t **stack, unsigned int line_number)
{
	...
	/* Check if the top element is within the
...
{
    (void)line_number; /* not used hence */
    ...
    if (*stack && (*stack)->next)
    {
        ...
        last->next = top;
        second->prev = NULL;
        *stack = second;   /* Update the stack pointer to the new top */
    }
}
...
{
  /* Check if the stack is long enough to perform the operation */
  if (*stack == NULL || (*stack)->next == NULL)
  {
